...
%         e1 = 1.1e-7 + (1.2e-7 - 1.1e-7) * rand;  %1.101e-7; 
    ...
    %置信区间 
    z_value = 1.645;  % 90%置信区间的Z值 1.645
    se2 = sqrt((survival_rate2 .* (1 - survival_rate2)) / total_simulations);  % 标准误差计算
    ci_lower2 = max(survival_rate2 - z_value * se2, 0);  % 下限不能小于0
    ci_upper2 = min(survival_rate2 + z_value * se2, 1);  % 上限不能大于1

    se4 = sqrt((survival_rate4 .* (1 - survival_rate4)) / total_simulations);  % 4维模型的标准误差
    ci_lower4 = max(survival_rate4 - z_value * se4, 0);
    ci_upper4 = min(survival_rate4 + z_value * se4, 1);

    se9 = sqrt((survival_rate9 .* (1 - survival_rate9)) / total_simulations);  % 9维模型的标准误差
    ci_lower9 = max(survival_rate9 - z_value * se9, 0);
    ci_upper9 = min(survival_rate9 + z_value * se9, 1);
    
    % 绘制生存曲线
    figure (1); 
    plot([0,dense_time2], [1,dense_survival_rate2], 'LineWidth', 1.5, 'Color', [0, 0, 1, 0.2]);  % 插值后的曲线
    hold on;
    plot([0,dense_time4], [1,dense_survival_rate4], 'LineWidth', 1.5, 'Color', [1, 0, 0, 0.2]);  % 插值后的曲线
    hold on;
    plot([0,dense_time9], [1,dense_survival_rate9], 'LineWidth', 1.5, 'Color', [0, 1, 0, 0.2]);  % 插值后的曲线
    hold on;
    fill([[0:1:total_time] fliplr([0:1:total_time])], [1 ci_upper2 fliplr([1 ci_lower2])], 'b', 'FaceAlpha', 0.05, 'EdgeColor', 'b','EdgeAlpha', 0.4); % 填充置信区间
    hold on;
    fill([[0:1:total_time] fliplr([0:1:total_time])], [1 ci_upper4 fliplr([1 ci_lower4])], 'r', 'FaceAlpha', 0.05, 'EdgeColor', 'r','EdgeAlpha', 0.4);  % 置信区间填充
    hold on;
    fill([[0:1:total_time] fliplr([0:1:total_time])], [1 ci_upper9 fliplr([1 ci_lower9])], 'g', 'FaceAlpha', 0.05, 'EdgeColor', 'g','EdgeAlpha', 0.4);  % 置信区间填充
...
%     death_counts9 = [5, 10, 0, 1, 0, 0];
    ...
    % 设置不同颜色
    fig(1).FaceColor = [0 0 1];  % 蓝色
    fig(2).FaceColor = [1 0 0];  % 红色
    fig(3).FaceColor = [0 1 0];  % 绿色

    % 设置透明度
    fig(1).FaceAlpha = 0.15;  % 设置第1组的填充透明度
    fig(2).FaceAlpha = 0.15;  % 设置第2组的填充透明度
    fig(3).FaceAlpha = 0.15;  % 设置第3组的填充透明度

%     % 在每个柱状图上方添加数值标签
%     for ii = 1:3  % 3种治疗策略
%         for jj = 1:6  % 每个时间段
%             % 获取每个柱子的x坐标和y值
%             x_pos = x(jj) + (ii - 2) * 0.2;  % 调整x位置，确保数值位于柱子上方
%             y_value = [death_counts2(jj), death_counts4(jj), death_counts9(jj)];  % 每组的死亡人数
